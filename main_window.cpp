...
main_window::main_window()
{
    ...
    checker_.load("data/words.txt");
    highlighter_ = new spell_checker_highlighter(editor->document(), checker_);
...
void main_window::setup_format_menu()
{
    ...
    connect(action_font, &QAction::triggered, this, [this] {
        ...
        const QFont current = cursor.hasSelection()
            ? cursor.charFormat().font()
            : editor->font();
        const QFont chosen = QFontDialog::getFont(&ok, current, this, "Select Font");
        if (!ok)
            return;
    ...
    connect(action_color, &QAction::triggered, this, [this] {
        const QColor color = QColorDialog::getColor(
            editor->textColor(), this, "Select Text Color");
        if (color.isValid())
...
void main_window::setup_view_menu()
{
    ...
    connect(action_zoom_in, &QAction::triggered, this, [this] { zoom_in(); });

    auto* action_zoom_out = view_menu->addAction("Zoom Out");
    action_zoom_out->setShortcut(QKeySequence("Ctrl+-"));
    connect(action_zoom_out, &QAction::triggered, this, [this] { zoom_out(); });
...
void main_window::update_status_bar()
{
    ...
    const int ln = cursor.blockNumber() + 1;
...
void main_window::show_context_menu(const QPoint& pos)
{
    ...
    const auto word_lower = selected_word.toLower().toStdString();

    if (!word_lower.empty() && !checker_.check(word_lower)) {
        const auto suggs = checker_.suggestions(word_lower);

        auto* first_action = menu->actions().isEmpty() ? nullptr : menu->actions().first();

        if (!suggs.empty()) {
            for (int i = static_cast<int>(suggs.size()) - 1; i >= 0; --i) {
                const auto sugg = QString::fromStdString(suggs[i]);
                auto* action = new QAction(sugg, menu);
                const int begin = cursor.selectionStart();
                const int end = cursor.selectionEnd();
                connect(action, &QAction::triggered, this, [this, begin, end, sugg] {
                    auto c = editor->textCursor();
                    c.setPosition(begin);
            ...
            auto* no_sugg = new QAction("(no suggestions)", menu);
            no_sugg->setEnabled(false);
            menu->insertAction(first_action, no_sugg);
        }
